...
class net_prot_gat(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear0 = nn.ModuleList([nn.Linear(256, 256) for _ in range(6)])
        self.linear1 = nn.ModuleList([nn.Linear(256, 256) for _ in range(6)])
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()
        self.w_attn = nn.ModuleList([nn.Linear(256, 256) for _ in range(6)])
        self.linear_final = nn.Linear(256, 256)

    def forward(self, x, adj):
        adj[:, list(range(512)), list(range(512))] = 1
        for l in range(6):#7个头
            x0 = x
...
def gentest474RNAdataframe():    with open(r'/home/xli/NABProt/task1ab/dataft/testdata/test474pos68.pkl', 'rb') as f1:
    ...
    with open(r'/home/xli/NABProt/task1ab/dataft/testdata/test474pos175.pkl', 'rb') as f4:
        testnegset2 = pickle.load(f4)
...
testset=gentest474RNAdataframe()#gentest474DNAdataframe()

test_loader = DataLoader(dataset=ProDataTestset(testset), batch_size=1, shuffle=False, num_workers=2)

model.load_state_dict(torch.load('/home/xli/NABProt/task1ab/a_retrainmodel/T5/resubmit/RNA/Hyperparameter_selection/layer6_drop/RNAmodel_6/epoch6RNA.pkl'))
